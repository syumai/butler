...
Picovoice was rejected because its registration requirements do not fit. sherpa-onnx 1.12.14 and the public GigaSpeech 3.3M KWS model are bundled into the APK, and the wake phrase is selectable from `Hello Computer` (default), `Hey Butler`, `Hello Butler`, and `Hello World`. Only the token sequence for the selected phrase is loaded, and the model is regenerated when the setting changes. No account, AccessKey, or additional model creation/download is required. The model runs on the device CPU. There is no fallback to cloud detection or an authentication server. The AAR and onnx model are not tracked in Git; Gradle automatically runs `scripts/fetch-deps.sh` to fetch and verify them (see [third_party/sherpa-onnx](../third_party/sherpa-onnx/README.md)).
