...
We use the int8 encoder and joiner, fp32 decoder, and original tokens.txt. Asset filenames are shortened to encoder.onnx, decoder.onnx and joiner.onnx. keywords.txt selects upstream HELLO WORLD. hello-computer.txt, hey-butler.txt and hello-butler.txt use the bundled model’s SentencePiece tokenizer to encode HELLO COMPUTER, HEY BUTLER and HELLO BUTLER. All files add result labels. hello-butler.txt (the currently selected phrase file) additionally has lines tuned for the Japanese pronunciation of "Hello Butler" ("ハロー、バトラー"), derived from how the model decodes synthetic Japanese-accented TTS audio and a real Japanese speaker's recording — see `docs/device-validation.md`. `scripts/eval-recording.py` can evaluate a phrase file against a real microphone recording of the wake phrase (plus negative recordings), the same way this file was last tuned. Only the selected phrase file is loaded. No runtime download or license-server connection is used. Gradle packages only armeabi-v7a.
