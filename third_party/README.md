...
| Julius 4.6 (`libjulius-bin.so`, experimental wake engine) | BSD-3-Clause | https://github.com/julius-speech/julius (see [third_party/julius](julius/)) |
...
See [third_party/vosk](vosk/) for vosk-android's, JNA's, and the Vosk Japanese model's detailed provenance, hashes, and license files, and [third_party/julius](julius/) for the same for Julius and the Julius Dictation Kit acoustic model (the experimental alternative wake engine, off by default — see Settings -> Wake). `app/src/main/res/values/strings.xml`'s `settings_licenses_body` (and the `values-ja` equivalent), shown in-app under "Third-party notices" / "配布物と出典", summarizes this same list for end users.
