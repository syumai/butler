...
See [third_party/sherpa-onnx](sherpa-onnx/) for sherpa-onnx's and the KWS model's detailed provenance, hashes, and license files (`sherpa-onnx`'s own LICENSE and the bundled ONNXRUNTIME-LICENSE). `app/src/main/res/values/strings.xml`'s `settings_licenses_body` (and the `values-ja` equivalent), shown in-app under "Third-party notices" / "配布物と出典", summarizes this same list for end users.
