...
`VoskWakeDecoder` restricts the recognizer to a runtime grammar built from `WakePhrase.HELLO_BUTLER.voskPhrases` plus the catch-all — `["ハロー バトラー", "[unk]"]` — via `Recognizer(model, 16000f, grammarJson)`, rather than running free-vocabulary recognition (see "Evaluation" below for why). A second pronunciation set, "ヘイ バトラー" ("Hey Butler"), was added 2026-09-17 and withdrawn the same day (see "Hey Butler tried and withdrawn" below); the grammar is back to the single phrase. A result is only treated as a wake-word hit when the recognized text contains one phrase's words as an **adjacent** run — `VoskWake.hit` (pure Kotlin, unit-tested in `VoskWakeTest.kt`) splits the result text on whitespace and checks that phrase's word list appears contiguously in the token list, and `VoskWakeDecoder` calls it once per phrase in `voskPhrases`. This matters because Vosk's partial results transiently surface a bare `ハロー` or a bare `バトラー` on unrelated speech even in grammar mode (confirmed against a real negative recording), so requiring only "both words present somewhere" would false-wake; requiring adjacency does not.
...
`vosk-model-small-ja-0.22` is a Japanese acoustic/language model; it does not reliably recognize the English pronunciation of "Hello Butler" as `ハロー バトラー`. Only the Japanese pronunciation ("ハロー、バトラー") wakes the app — saying it in English is not detected. There is no other on-device engine to fall back to for English-pronunciation coverage.

## "Hey Butler" tried and withdrawn (2026-09-17)

The wake phrase is "Hello Butler" only. A second pronunciation set, "ヘイ バトラー" ("Hey Butler"), was added to `WakePhrase.HELLO_BUTLER.voskPhrases` on 2026-09-17 after clearing an offline evaluation with zero false wakes (Vosk's grammar-restricted adjacency check, same rule as `ハロー バトラー`); the same day, an equivalent addition to Julius's phone-loop grammar was also evaluated and shipped, gated on both `cmscore1 >= 0.05` and a filler-word-count cap (see `scripts/julius-wake/README.md`'s "2026-09-17: Hey Butler, take two — a structural gate" section and `third_party/julius/README.md`). Both offline results had flagged recall for a real speaker actually saying "Hey Butler" as unverified, and real-world on-device use that same day confirmed the concern the other way: "Hey Butler" false-woke too often in practice on both engines. It was withdrawn back to "Hello Butler" only — `WakePhrase.HELLO_BUTLER.voskPhrases = listOf("ハロー バトラー")` — the same day; see `scripts/julius-wake/README.md`'s and `third_party/julius/README.md`'s own closing notes for the Julius side, including why the filler-count gate it added stayed even though the word it was built for didn't.
