...
The same phrase was also tried for Julius (the default engine)'s phone-loop grammar and evaluated offline (`scripts/julius-eval.py`, see `scripts/julius-wake/README.md`'s "2026-09-17: Hey Butler" section and `third_party/julius/README.md`) — that attempt produced false wakes on real Japanese speech (LibriVox) even after dropping its most confusable pronunciation variants, so it was **not** shipped for Julius; `WakePhrase.juliusWords` still has only `"ハローバトラー"`. Vosk's own addition here was not re-evaluated with an equivalent real-speech sweep beyond the unit tests (`VoskWakeTest.kt`), but the different detection mechanism — an exact two-word adjacency match in a final ASR result, restricted to a two-entry runtime grammar, versus Julius's phone-loop confidence race against an open-ended filler category — is why it was judged safe to ship without repeating that sweep: the same reasoning that made `["ハロー バトラー", "[unk]"]` reliable (see "Evaluation" above) applies equally to a third grammar entry. As a result, **"Hey Butler" currently only wakes the app when Vosk is selected in Settings → Wake** (Julius, the default, still only detects "Hello Butler").
